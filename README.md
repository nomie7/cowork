...
- **Per-Message Feedback Loop** — opt-in 👍 / 👎 + comment on every assistant reply. 👍 tells the synthesiser "capture this procedure"; 👎 with a comment triggers a focused **remediation pass** that picks the auto-skill most likely responsible (token-overlap match) and rewrites just that one skill to address the complaint. 👎 alone is treated as "do not distil from this chat."
...
2. **Candidate selection.** Sessions whose `updatedAt` is newer than the saved cursor are summarised: first 6 user prompts (≤600 chars each), the last assistant reply (≤3000 chars), and any per-message 👍 / 👎 feedback with comments. Sessions whose final reply looks like an error (rate limit, missing API key, connection error) are silently dropped, as are sessions with no user or assistant turn yet.
3. **Pass 1 — Remediation (👎 with a comment).** For each session containing an assistant message marked 👎 with a non-empty comment, a token-overlap heuristic picks the auto-skill whose name + description + first 4 KB of `SKILL.md` best matches the disliked excerpt and the user's complaint (minimum score = 2 distinct content tokens). The model is then asked to rewrite *that one skill* to address the complaint, or `{"skip": true}` if it judges the wrong target was picked. Capped at **5 remediations per run**; the rest defer to the next tick. Successful remediations consume the session, so Pass 2 will not also distil from it.
4. **Pass 2 — Synthesis.** The remaining candidates are sent to the TigerBot model with the current skill list. The synthesiser returns strict JSON proposals, each either:
   ...
   The synthesiser is told to skip casual chats / one-off Q&A that don't generalise — quality > quantity. 👍 reinforces "capture this"; 👎 explicitly suppresses distillation.
...
| 👎 + comment | Picks an auto-skill via token-overlap, asks the model to rewrite it to address the complaint | Skipped (already consumed) |
